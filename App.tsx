...
const AppContent: React.FC = () => {
  ...
  const handleAddUser = async (name: string, role: UserRole, password: string) => {
    ...
    await setDoc(doc(db, 'clinic_data', user?.clinicId || 'c1'), { users: newList }, { merge: true });
  };

  if (!user) return (<div className="min-h-screen max-w-md mx-auto bg-slate-50 shadow-2xl relative flex flex-col font-sans border-x border-slate-100 overflow-hidden"><LoginPage allUsers={allUsers} onLogin={handleLogin} onAddUser={handleAddUser} /></div>);
