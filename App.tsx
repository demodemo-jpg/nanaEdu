...
// --- Types & Config ---
import { UserRole, SkillLevel, SkillLevelLabels, UserSkillProgress, Procedure, Skill, User } from './types';
import { MOCK_PROCEDURES, MOCK_SKILLS } from './constants';
    ...
    <div className="min-h-screen max-w-md mx-auto bg-slate-50 shadow-2xl relative flex flex-col font-sans border-x border-slate-100 overflow-hidden">
      ...
      <main className="flex-1 overflow-y-auto no-scrollbar">
        <Routes>
          ...
          <Route path="/procedures" element={<div className="p-8">手順書リスト (実装済み)</div>} />
          <Route path="/skills" element={<div className="p-8"><button onClick={() => navigate('/skills/u1')}>田中さんのスキルマップへ</button></div>} />
          ...
          <Route path="/qa" element={<div className="p-10 text-center">Q&A 機能</div>} />
          <Route path="/profile" element={<div className="p-10 text-center"><button onClick={handleLogout} className="px-10 py-4 bg-red-500 text-white rounded-3xl font-black shadow-lg">ログアウト</button></div>} />
        </Routes>
      </main>
      {user && <Navigation />}
      {isSaving && (
        <div className="fixed bottom-24 left-1/2 -translate-x-1/2 bg-slate-900 text-white px-6 py-3 rounded-full text-[9px] font-black tracking-widest flex items-center gap-3 shadow-2xl animate-in slide-in-from-bottom-2">
          <PartyPopper size={14} className="text-emerald-400" /> DATA SYNCED ACROSS DEVICES
        </div>
      )}
    </div>
  );
